...
int calculeazaInaltimeArbore(nod* arbore) {
	...
	if (arbore != NULL) {
		int inaltimeStg = calculeazaInaltimeArbore(arbore->stanga);
		int inaltimeDr = calculeazaInaltimeArbore(arbore->dreapta);
		return((inaltimeStg > inaltimeDr) ? inaltimeStg : inaltimeDr)+1;
	}
...
int calculeazaGradEchilibru(nod* arbore) {
	...
	int GE = calculeazaInaltimeArbore(arbore->stanga) - calculeazaInaltimeArbore(arbore->dreapta);
	return GE;
}
